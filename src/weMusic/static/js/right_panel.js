...
function pin_song(event) {
    song_index = $(event.target).parent().parent().attr('order');
    $.post("/pin_song/" + room_key + '/' + parseInt(song_index));
}

function like_song(event) {
    song_index = $(event.target).parent().parent().attr('order');
    $.post("/like_song/" + room_key + '/' + parseInt(song_index));
}
